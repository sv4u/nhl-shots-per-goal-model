...
```{r}
ovechkin = get_skater_data(analysis_2017, "Alex Ovechkin")
hall = get_skater_data(analysis_2017, "Taylor Hall")
khaira = get_skater_data(analysis_2017, "Jujhar Khaira")
```

Now, we can calculate SPG.

```{r}
ovechkin_spg = calculate_spg(ovechkin)
hall_spg = calculate_spg(hall)
khaira_spg = calculate_spg(khaira)
```
...
| Player | SPG |
| ------ | --- |
| Ovechkin | `r ovechkin_spg` |
| Hall | `r hall_spg` |
| Khaira | `r khaira_spg` |

Though Jujhar Khaira and Alex Ovechkin have completely different play styles, we see that both have a very similar SPG. Hall, a pure scoring winger, has a smaller SPG, showing that he has a more accurate shot or that he has increased scoring chances and converts on them more often that Ovechkin and Khaira.

Now, let's calculate their danger level SPG.

```{r}
ovechkin_low_spg = low_danger_spg(ovechkin)
ovechkin_medium_spg = medium_danger_spg(ovechkin)
ovechkin_high_spg = high_danger_spg(ovechkin)

hall_low_spg = low_danger_spg(hall)
hall_medium_spg = medium_danger_spg(hall)
hall_high_spg = high_danger_spg(hall)

khaira_low_spg = low_danger_spg(khaira)
khaira_medium_spg = medium_danger_spg(khaira)
khaira_high_spg = high_danger_spg(khaira)
```

Let's look at the SPGs now.

| Player | Low Danger SPG | Medium Danger SPG | High Danger SPG |
| ------ | -------------- | ----------------- | --------------- |
| Ovechkin | `r ovechkin_low_spg` | `r ovechkin_medium_spg` | `r ovechkin_high_spg` |
| Hall | `r hall_low_spg` | `r hall_medium_spg` | `r hall_high_spg` |
| Khaira | `r khaira_low_spg` | `r khaira_medium_spg` | `r khaira_high_spg` |

As we can see, Ovechkin has scored from the point, while Hall and Khaira have not. This matches both Hall's and Khaira's play style, as both do not have a slapshot like Ovechkin.
...
```{r}
kessel = get_skater_data(analysis_2017, "Phil Kessel")
kucherov = get_skater_data(analysis_2017, "Nikita Kucherov")
tuch = get_skater_data(analysis_2017, "Alex Tuch")
```

Now for the seasonal SPGs:

```{r}
kessel_spg = calculate_spg(kessel)
kucherov_spg = calculate_spg(kucherov)
tuch_spg = calculate_spg(tuch)
```

Here's what they did over the 2017-2018 season.

| Player | SPG |
| ------ | --- |
| Kessel | `r kessel_spg` |
| Kucherov | `r kucherov_spg` |
| Tuch | `r tuch_spg` |

As we can see, Kessel is much like Hall, a pure goal scorer. Kucherov is exceptional at scoring as well as passing, as his SPG shows. Alex Tuch on the other had is a young player who is just breaking into the league.

Let's calculate the danger level SPGs now.

```{r}
kessel_low_spg = low_danger_spg(kessel)
kessel_medium_spg = medium_danger_spg(kessel)
kessel_high_spg = high_danger_spg(kessel)

kucherov_low_spg = low_danger_spg(kucherov)
kucherov_medium_spg = medium_danger_spg(kucherov)
kucherov_high_spg = high_danger_spg(kucherov)

tuch_low_spg = low_danger_spg(tuch)
tuch_medium_spg = medium_danger_spg(tuch)
tuch_high_spg = high_danger_spg(tuch)
```

And now let's look at what the data shows:

| Player | Low Danger SPG | Medium Danger SPG | High Danger SPG |
| ------ | -------------- | ----------------- | --------------- |
| Kessel | `r kessel_low_spg` | `r kessel_medium_spg` | `r kessel_high_spg` |
| Kucherov | `r kucherov_low_spg` | `r kucherov_medium_spg` | `r kucherov_high_spg` |
| Tuch | `r tuch_low_spg` | `r tuch_medium_spg` | `r tuch_high_spg` |

As we can see, it is uncommon for these players to take shots from the point. Tuch does have success from the point, but I believe that success is short lived as it should increase as he plays more games.

### Defensemen

Similar to the center analysis, we will just look at SPG over an entire season and SPG at danger locations.

Let's load in our defensemen's data.

```{r}
burns = get_skater_data(analysis_2017, "Brent Burns")
klingberg = get_skater_data(analysis_2017, "John Klingberg")
doughty = get_skater_data(analysis_2017, "Drew Doughty")
maatta = get_skater_data(analysis_2017, "Olli Maatta")
engelland = get_skater_data(analysis_2017, "Deryk Engelland")
```

Now, we can calculate SPG.

```{r}
burns_spg = calculate_spg(burns)
klingberg_spg = calculate_spg(klingberg)
doughty_spg = calculate_spg(doughty)
maatta_spg = calculate_spg(maatta)
engelland_spg = calculate_spg(engelland)
```

Here's how these defenseman shored up:

| Player | SPG |
| ------ | --- |
| Burns | `r burns_spg` |
| Klingberg | `r klingberg_spg` |
| Doughty | `r doughty_spg` |
| Maatta | `r maatta_spg` |
| Engelland | `r engelland_spg` |

Let's also calculate SPG by danger level.

```{r}
burns_low_spg = low_danger_spg(burns)
burns_medium_spg = medium_danger_spg(burns)
burns_high_spg = high_danger_spg(burns)

klingberg_low_spg = low_danger_spg(klingberg)
klingberg_medium_spg = medium_danger_spg(klingberg)
klingberg_high_spg = high_danger_spg(klingberg)

doughty_low_spg = low_danger_spg(doughty)
doughty_medium_spg = medium_danger_spg(doughty)
doughty_high_spg = high_danger_spg(doughty)

maatta_low_spg = low_danger_spg(maatta)
maatta_medium_spg = medium_danger_spg(maatta)
maatta_high_spg = high_danger_spg(maatta)

engelland_low_spg = low_danger_spg(engelland)
engelland_medium_spg = medium_danger_spg(engelland)
engelland_high_spg = high_danger_spg(engelland)
```

Now, we can compare all the defensemen.

| Player | Low Danger SPG | Medium Danger SPG | High Danger SPG |
| ------ | -------------- | ----------------- | --------------- |
| Burns | `r burns_low_spg` | `r burns_medium_spg` | `r burns_high_spg` |
| Klingberg | `r klingberg_low_spg` | `r klingberg_medium_spg` | `r klingberg_high_spg` |
| Doughty | `r doughty_low_spg` | `r doughty_medium_spg` | `r doughty_high_spg` |
| Maatta | `r maatta_low_spg` | `r maatta_medium_spg` | `r maatta_high_spg` |
| Engelland | `r engelland_low_spg` | `r engelland_medium_spg` | `r engelland_high_spg` |
