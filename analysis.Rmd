...
After much thinking, I have determined to use infinity as the shutout SPG.
...
get_low_danger = function(data) {
	subset(data,
		   x_adj >= 20 & x_adj <= 35 & y_adj <= 43 & y_adj >= -43)
}

get_medium_danger = function(data) {
	subset(data,
		   x_adj >= 32 & x_adj <= 82 &
		   	((y_adj >= 7 & y_adj <= 43) | (y_adj <= -7 & y_adj >= -43)))
}

get_high_danger = function(data) {
	subset(data,
		   x_adj >= 40 & x <= 90 & y_adj >= -22 & y_adj <= 22)
}
```
...
Let's now look at game by game SPG.

```{r}
crosby_spg_games = get_spg_games(crosby)
mcdavid_spg_games = get_spg_games(mcdavid)
kopitar_spg_games = get_spg_games(kopitar)
dvorak_spg_games = get_spg_games(dvorak)
```

Let's compare Sidney Crosby and Connor McDavid.
...
analysis_2016 = data.frame(x = regular_season_2016$xCord,
						   ...
						   goalie_name =
						   	regular_season_2016$goalieNameForShot,
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
plot = plot_spg_game(murray_spg_games_2016,
					 "#000000",
					 "#FCB514",
					 "Matt Murray (2016)")
plot
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
plot = plot_spg_game(murray_spg_games_2017,
					 "#000000",
					 "#FCB514",
					 "Matt Murray (2017)")
plot
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
plot = plot_spg_game(murray_spg_games_2018,
					 "#000000",
					 "#FCB514",
					 "Matt Murray (2018)")
plot
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
plot = plot_spg_game(desmith_spg_games_2017,
					 "#000000",
					 "#FCB514",
					 "Casey DeSmith (2017)")
plot
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
plot = plot_spg_game(desmith_spg_games_2018,
					 "#000000",
					 "#FCB514",
					 "Casey DeSmith (2018)")
plot
